...
#
# Besonderheit dieses Repos: Der Web-Baum liegt im Unterordner naehrtakt-web/.
# Das Skript wechselt dorthin ($webRoot) und spiegelt naehrtakt-web/<pfad> nach
# /naehrtakt/<pfad>. Alles ausserhalb (supabase/, tools/, DATA-SCHEMA.md,
# OPERATOR-KEY.local.txt) wird dadurch NICHT angefasst.
#
# ⛔ NIE mit hochgeladen:
...
#    - deploy-local.log
#
# Lehren (uebernommen aus lagerverwaltung/punkto):
...
#  - Verify mit Browser-User-Agent (hCDN kann Nicht-Browser eine Bot-Challenge statt der
#    echten Datei ausliefern -> falsche Groesse/Titel).
#
...
#
# Aufruf:
#   powershell -ExecutionPolicy Bypass -File "C:\Users\karbi\Documents\Claude Code\naehrtakt\deploy-local.ps1"

$ErrorActionPreference = 'Stop'
$scriptRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
$root = Join-Path $scriptRoot 'naehrtakt-web'   # Web-Baum = Upload-Wurzel
if (-not (Test-Path $root)) { Write-Host "naehrtakt-web/ nicht gefunden neben dem Skript."; exit 1 }
Set-Location $root

$FTPHOST = 'ftp.vaydena.de'
$FTPUSER = 'u424339903.deploy'
$FTPDIR  = 'naehrtakt'
$SITE    = 'https://naehrtakt.vaydena.de'
...
$log  = Join-Path $scriptRoot 'deploy-local.log'    # ausserhalb von naehrtakt-web/, git-ignoriert
$errf = Join-Path $env:TEMP 'nt-curl-stderr.txt'
...
function Listing([string]$title) {
  ...
  foreach ($d in @('', 'assets/', 'assets/data/', 'assets/img/')) {
    Log "-- /$FTPDIR/$d"
...
# --- 2) Dateiliste (⛔ *.local.txt / *.md / deploy-local.log NIE deployen) ---
$excludeName = @('deploy-local.log')
$files = @(Get-ChildItem -Recurse -File | Where-Object {
  ($excludeName -notcontains $_.Name) -and ($_.Name -notlike '*.local.txt') -and ($_.Name -notlike '*.md')
})
$total = $files.Count
...
$textExt = @('.html', '.css', '.js', '.txt', '.webmanifest', '.json')
$bad = 0
foreach ($f in $files) {
  $rel = Rel $f
